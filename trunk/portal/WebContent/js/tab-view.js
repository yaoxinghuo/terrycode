	...
	************************************************************************************************************/		
	...
	function tabClick()
	{
		...
		var position = idArray[idArray.length-1].replace(/[^0-9]/gi,'');
		_currentPosition = position;
		showTab(this.parentNode.parentNode.id,position);
		if($("tab"+position).innerHTML!="") {
			initTabDrag(position);
			initMarquee(position);
		}
		else
			addTemplateToTab(position);
	}
	...
	function getTabIndexByTitle(tabTitle)
	{
		...
		for(var prop in dhtmlgoodies_tabObj){
			if(prop.startsWith("dhtmlgoodies")) {
