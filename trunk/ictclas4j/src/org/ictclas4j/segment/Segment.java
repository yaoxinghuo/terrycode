...
public class Segment {
	...
	private Segment() {
		String path = Constants.DATA_PATH != null
				&& !Constants.DATA_PATH.equals("") ? Constants.DATA_PATH
				: "./data/";
		if (!path.endsWith("/"))
			path = path + "/";
		...
		personTagger = new PosTagger(Utility.TAG_TYPE.TT_PERSON, path + "nr",
				coreDict);
		transPersonTagger = new PosTagger(Utility.TAG_TYPE.TT_TRANS_PERSON,
				path + "tr", coreDict);
		placeTagger = new PosTagger(Utility.TAG_TYPE.TT_TRANS_PERSON, path
				+ "ns", coreDict);
		lexTagger = new PosTagger(Utility.TAG_TYPE.TT_NORMAL, path + "lexical",
				coreDict);
